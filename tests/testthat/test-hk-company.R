...
test_that("hk_com_hot_ss_ha response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_hot_ss_ha.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("hk_com_mutual_market response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_mutual_market.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("hk_com_fundmtl_bank response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_fundmtl_bank.json"))
...
test_that("hk_com_fundmtl_insurance response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_fundmtl_insurance.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("hk_com_fundmtl_reit response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_fundmtl_reit.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("hk_com_fundmtl_other_financial response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_fundmtl_other_financial.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("hk_com_fs_bank response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_fs_bank.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("hk_com_fs_insurance response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_fs_insurance.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("hk_com_fs_security response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "hk_com_fs_security.json"))
