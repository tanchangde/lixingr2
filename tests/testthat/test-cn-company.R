...
test_that("cn_com_operating_data response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_operating_data.json"))
...
test_that("cn_com_dividend response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_dividend.json"))
...
test_that("cn_com_customers response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_customers.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("cn_com_suppliers response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_suppliers.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("cn_com_fundmtl_non_financial response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_fundmtl_non_financial.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("cn_com_fundmtl_bank response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_fundmtl_bank.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("cn_com_fs_non_financial response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_fs_non_financial.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("cn_com_fs_bank response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_fs_bank.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("cn_com_fs_insurance response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_fs_insurance.json"))

  expect_true(is.data.frame(result))
  expect_true(all(purrr::map_lgl(result, ~ !is.list(.))))
})

test_that("cn_com_fs_other_financial response is correctly converted to a data frame", {
  result <- mock_lxr_unnest(test_path("fixtures", "cn_com_fs_other_financial.json"))
